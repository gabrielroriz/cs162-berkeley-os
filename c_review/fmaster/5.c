...
int write_and_print(const char *response, int socket_fd) {
    printf("%s", response);
...
int handle_req(char *request, int socket_fd) {
    ...
    if (path == NULL) {
        ...
        write_and_print("HTTP/1.1 400 Bad Request\n\n", socket_fd);
        return -1;
    }
    ...
    if (fd == -1) {
        // 👉 Change this to send an the actual response to the socket.
        write_and_print("HTTP/1.1 404 Not Found\n\n", socket_fd);
        if (errno == ENOENT) {
            ...
            write_and_print("HTTP/1.1 500 Internal Server Error\n\n", socket_fd);
        }
    ...
    while (1) {
        ...
        if (req_socket_fd >= 0) {
            ...
            if (bytes_read < MAX_REQUEST_BYTES) {
                ...
                // 👉 Change this to send an the actual response to the socket.
                write_and_print("HTTP/1.1 413 Content Too Large\n\n", socket_fd);
            }
